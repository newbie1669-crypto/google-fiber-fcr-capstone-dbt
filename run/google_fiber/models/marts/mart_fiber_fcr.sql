    ...
    create or replace table `gbi-test`.`dbt_prod`.`mart_fiber_fcr`
...
WITH combined AS (
    SELECT * FROM `gbi-test`.`dbt_prod`.`stg_market_1`
    UNION ALL
    SELECT * FROM `gbi-test`.`dbt_prod`.`stg_market_2`
    UNION ALL
    SELECT * FROM `gbi-test`.`dbt_prod`.`stg_market_3`
),
